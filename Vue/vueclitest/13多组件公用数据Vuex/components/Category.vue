<template>
  <div class="category">
    <h4>人员列表的和{{ personList.length }}</h4>
    <h2>当前的计算结果为{{ $store.state.sum }}</h2>
    ...
    <button @click="addodd">技术不加</button>
    <button @click="addwait">延迟1秒加</button>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    subtraction() {
      // this.$store.dispatch("subtraction", this.num);
